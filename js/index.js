...
function indexLogo() {
    ...
    function bossMove() {
        ...
        function fn() {
            ...
            moving({
                obj: boss,
                attrs: {top: 20, left: 300 + 60 * n},
                duration: 200,
                callBack: function () {
                    n++;
                    moving({
                        obj: boss,
                        attrs: {top: 80, left: 300 + 60 * n},
                        duration: 200,
                        callBack: function () {
                            if (n <= 8) {
                                fn();
...
function victoryFn() {
    ...
    setTimeout(function () {
        moving({
            obj: character,
            attrs: {top: 327, width: 0, height: 0, left: 397},
            duration: 1000,
            callBack: function () {
                //星星
                ...
                stars.timer = setInterval(function () {
                    ...
                    moving({
                        obj: ii,
                        attrs: {left: pos.left, top: pos.top, width: pos.size, height: pos.size},
                        duration: 1800,
                        fx: 'easeIn',
                        callBack: function () {
...
function trans(obj, fn) {
    ...
    transition.style.left = the_background.offsetLeft+ 'px';
...
function deadFn() {
    ...
    timer = setInterval(function () {
        ems.forEach(function (a) {
            moving({
                obj: a,
                attrs: {height: 0, top: 110},
                duration: 50,
                callBack: function () {
                    eyes.play();
                    moving({
                        obj: a,
                        attrs: {height: 10, top: 100},
                        duration: 50
                    });
                }
            });
    ...
    moving({
        obj: bg,
        attrs: {width: 3500, height: 2400, left: -1350, bottom: -1000},
        duration: 1800,
        callBack: function () {
...
function mainFn() {
    ...
    function checkpoint(j) {
        ...
        function watering() {
            ...
            moving({
                obj: water,
                attrs: {top: 384, width: 60, height: 90, left: 100},
                duration: 1200,
                callBack: function () {
                    setTimeout(function () {
...
//      Explain();
     ...
     * novice:接收布尔值    true为新手教程   false或默认为正常模式(和街机不可同时存在);
     * pattern:接收布尔值   true为街机模式   false或默认为正常模式,     街机模式时:num属性失效,关卡为所有关卡的组合（不包括新手教程）
     * */
